...
import skbio as sk
from scmomentum.utilities import *
...
def network_distance(networks, resc=True):
    ...
    for i in range(0, nc):
        for j in range(i, nc):
            ...
            w1, w2 = cdm_equalDims([m1, m2])

            if dis_type == "euclidean":
                dist.at[i, j] = np.linalg.norm(w1 - w2)
                dist.at[j, i] = dist.at[i, j]

            elif dis_type == "ji_all_genes":
                dist.at[i, j] = cdm_jaccard(m1.index.tolist(), m2.index.tolist())
                dist.at[j, i] = dist.at[i, j]

            elif dis_type == "ji_top_edeges":
                dist.at[i, j] = nsm_edgeji(w1, w2)
                dist.at[j, i] = dist.at[i, j]

            elif dis_type == "lp_spectral":
                dist.at[i, j] = nsm_laplacianEvalues(w1, w2)
                dist.at[j, i] = dist.at[i, j]

            elif dis_type == "lp_egvectors":
                dist.at[i, j] = nsm_laplacianEvectors(w1, w2)
                dist.at[j, i] = dist.at[i, j]

            elif dis_type == "induced2norm":
                dist.at[i, j] = nsm_induced2norm(w1, w2)
                dist.at[j, i] = dist.at[i, j]

    if resc:
        dist = cdm_rescale(dist)
